...
function printMealList(day) {
  const doprint = (apps) => {
  const { sat, sun } = getWeekendDates();
  const satFull = 'Saturday, ' + sat;
  const sunFull = 'Sunday, '   + sun;
  const sections = [];
  if (day === 'sat' || day === 'both') sections.push(buildPrintSection(satFull, apps.filter(a => a.sat === true)));
  if (day === 'sun' || day === 'both') sections.push(buildPrintSection(sunFull, apps.filter(a => a.sun === true)));
    document.getElementById('printSection').innerHTML = `
      ...
      ${sections.join('')}
      <div class="print-footer">Laforêt · Staff Access Only</div>`;
    window.print();
  };
...
function buildPrintSection(dayLabel, people) {
    ...
    ? people.map((p, i) => {
        ...
        return `<div class="print-staff-row">
          ...
          <div class="print-line">
            ...
            <span class="print-info">${infoText}</span>
          </div>
        </div>`;
      }).join('')
    : '<div class="print-empty">No staff attending</div>';
  return `<div class="print-day-section">
    <div class="print-day-heading">
      <div class="print-day-label">${dayLabel}</div>
      <div class="print-day-count">${people.length} attending</div>
    </div>
